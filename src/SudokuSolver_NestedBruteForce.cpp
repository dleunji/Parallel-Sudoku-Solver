#include "SudokuSolver_NestedBruteForce.hpp"
#include <iostream>

SudokuSolver_NestedBruteForce::SudokuSolver_NestedBruteForce(SudokuBoard &board, bool print_message /*=true*/)
    : SudokuSolver(board)
{
...
void SudokuSolver_NestedBruteForce::solve_kernel(int row, int col)
{
    if (_solved)
    {
        return;
    ...
    {
        show_progress_bar(_board, _recursionDepth);
    }
